...
export class WorkActivityStore {
	...
	/**
	 * 拡張ホスト起動時点のチェックポイント。
	 * 起動直後のフォーカス等で上書きされる前の最終操作時刻を bootstrap が使う。
	 */
	private readonly bootCheckpoint: WorkActivityCheckpoint | undefined;

	constructor(private readonly globalState: vscode.Memento) {
		this.bootCheckpoint = this.readCheckpoint();
	}
	...
	getSharedLastActivityMs(username: string): number | null {
		return this.checkpointToMs(this.getCheckpoint(), username);
	}

	/**
	 * 起動時点で残っていた最終操作時刻（ms）。起動後の操作では変わらない。
	 */
	getBootLastActivityMs(username: string): number | null {
		return this.checkpointToMs(this.bootCheckpoint, username);
	}
	...
	private readCheckpoint(): WorkActivityCheckpoint | undefined {
	...
	private checkpointToMs(
		checkpoint: WorkActivityCheckpoint | undefined,
		username: string,
	): number | null {
